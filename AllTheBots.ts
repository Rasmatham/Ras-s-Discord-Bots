...
glados.on(Events.InteractionCreate, (interaction) => {
	...
	if (interaction.isChatInputCommand()) {
		switch (interaction.commandName) {
			...
			case `userinfo`: {
				info.userInfo({ interaction }).then((message) => {
					interaction.reply(message).catch(genericCatch);
				}).catch(genericCatch);
				break;
			}
			case `xkcd`: {
...
amber.on(Events.ClientReady, () => {
	amber.users.fetch(`707188499153158204`).then((user) => {
		if (amber.user !== null) {
			amber.user.setAvatar(user.avatarURL()).catch(() => {
				console.warn(`[${user.tag}] You're probably changing the avatar too fast`);
			});
			amber.user.setUsername(user.username).catch(() => {
				console.warn(`[${user.tag}] You're probably changing the username too fast`);
			});
		}
	}).catch(genericCatch);
});
amber.on(Events.UserUpdate, (oldUser, newUser) => {
	if (amber.user !== null) {
		if (newUser.id === `707188499153158204`) {
			amber.user.setAvatar(newUser.avatarURL()).catch(() => {
				console.warn(`[${newUser.tag}] You're probably changing the avatar too fast`);
			});
			amber.user.setUsername(newUser.username).catch(() => {
				console.warn(`[${newUser.tag}] You're probably changing the username too fast`);
			});
		}
	}
});
...
// #region CroissantBot
croissant.on(Events.MessageCreate, (message) => {
	forwarding.messageForwarding([{ message }]);
});
// #endregion
