...
const setup = (interaction: CommandInteraction):void => {
	mkdirSync(`./${interaction.client.user.username}/userinfo/${interaction.user.id}`);
	mkdirSync(`./${interaction.client.user.username}/userinfo/${interaction.user.id}/coinflip/`);
	writeFileSync(`./${interaction.client.user.username}/userinfo/${interaction.user.id}/coinflip/wins.log`, `0`);
	writeFileSync(`./${interaction.client.user.username}/userinfo/${interaction.user.id}/coinflip/losses.log`, `0`);
};
export const flip = (interaction: CommandInteraction):void => {
	const side = interaction.options.get(`side`).value as `${`heads`|`tails`}`;
	const coinflippath = `./${interaction.client.user.username}/userinfo/${interaction.user.id}/coinflip/`;
	const coinfilew = `./${interaction.client.user.username}/userinfo/${interaction.user.id}/coinflip/wins.log`;
	const coinfilel = `./${interaction.client.user.username}/userinfo/${interaction.user.id}/coinflip/losses.log`;
