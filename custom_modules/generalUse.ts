...
import { AttachmentBuilder, channelMention, ChannelType, EmbedBuilder, Events, formatEmoji, GatewayIntentBits, IntentsBitField, roleMention, SlashCommandBuilder, TextChannel, time, TimestampStyles, User, userMention, WebhookClient } from "discord.js";
...
// eslint-disable-next-line one-var
export const genericCatch = (err: unknown): void => {
	console.error(err);
};
// #endregion
				...
				webHooks.forEach((webHook: Webhook) => {
					if (webHook.owner instanceof User) {
						if (webHook.owner.id === user.id) {
							...
							inObj.sendTo.createWebhook({ name: `${user.username}-Webhook` }).then(() => {
								webHookFunction();
							}).catch((err: unknown) => {
								console.error(err);
								if (inObj.message.channel.type === ChannelType.GuildText)
		...
		inObj.bots.forEach((bot) => {
			...
			if (testMode ?? false) {
				bot.on(Events.Error, (err) => {
					console.error(`ERROR ${bot.user?.username ?? ``}`);
					console.error(err);
					console.error(``);
				});
				bot.on(Events.Debug, (deb) => {
					console.info(`DEBUG ${bot.user?.username ?? ``}`);
					console.info(deb);
					console.info(``);
				});
			}
