import { CommandInteraction, User, MessageButton, MessageActionRow, ButtonInteraction, Message, GuildMember, MessageMentions, UserResolvable, MessageActionRowComponent, EmojiResolvable, Emoji, Interaction, EmojiIdentifierResolvable, MessageButtonStyle } from "discord.js";
export const ticTacToe = (interaction: CommandInteraction):void => {
	interaction.guild.members.fetch(interaction.options.get(`playertwo`).user).then((playerTwo):void => {
		const movePieces:boolean = interaction.options.get(`movepieces`).value as boolean;
	...
	const button = (ID: string):MessageButton => {return new MessageButton().setCustomId(ID).setEmoji(`<:ras:741303046574702652>`).setStyle(`SECONDARY`)}
	
	const row1:MessageActionRow = new MessageActionRow().addComponents([button(`TTT1`), button(`TTT2`), button(`TTT3`)]);
	const row2:MessageActionRow = new MessageActionRow().addComponents([button(`TTT4`), button(`TTT5`), button(`TTT6`)]);
	const row3:MessageActionRow = new MessageActionRow().addComponents([button(`TTT7`), button(`TTT8`), button(`TTT9`)]);
	
	const startingGrid:MessageActionRow[] = [row1, row2, row3];
		...
		if(buttonInteraction.message.mentions instanceof MessageMentions){
			switch(buttonInteraction.customId){
				...
				if(buttonInteraction.user.id == buttonInteraction.message.mentions.members.first().id){
					...
					const players:[User, User] = [buttonInteraction.message.mentions.users.first(), buttonInteraction.message.mentions.users.last()]
					const randomNumber:number = Math.round(Math.random())
					const randomOrder:[User, User] = [players[randomNumber], players[1-randomNumber]]
				...
				if(buttonInteraction.customId.startsWith(`TTT`)){
					...
					.catch(console.error)
					.then(():void => {
						const players:[GuildMember, GuildMember] = [buttonInteraction.guild.members.cache.get(buttonInteraction.message.content.split(` `)[3].replace(`<@`, ``).replace(`>`, ``).replace(`!`, ``) as `${bigint}`), buttonInteraction.guild.members.cache.get(buttonInteraction.message.content.split(` `)[6].replace(`<@`, ``).replace(`>`, ``).replace(`!`, ``) as `${bigint}`)]
						const movePieces:boolean = !(buttonInteraction.message.content.includes(`no`))
						...
						if(buttonInteraction.user.id == players[0].id && buttonInteraction.component instanceof MessageButton){
							if(buttonInteraction.component.emoji.id == `741303046574702652`){
								...
								if(!movePieces){
									...
									let row1:MessageActionRow = new MessageActionRow().addComponents([newButton(0, 0), newButton(0, 1), newButton(0, 2)]);
									let row2:MessageActionRow = new MessageActionRow().addComponents([newButton(1, 0), newButton(1, 1), newButton(1, 2)]);
									let row3:MessageActionRow = new MessageActionRow().addComponents([newButton(2, 0), newButton(2, 1), newButton(2, 2)]);
									...
									const newButtons:MessageActionRow[] = [row1, row2, row3]
