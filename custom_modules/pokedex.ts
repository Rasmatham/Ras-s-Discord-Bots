...
type pokeObjType = {name: string, nat:string, types:[{name: string, color: string}, {name: string, color: string}]|[{name: string, color: string}]}
//dex embed
export const natDex = (Query: string|number):ReplyMessageOptions => {
	let dexNumber = 0 as pokeRange;
	if (!isNaN(Number(Query))) { Query = Number(Query); }
	switch (typeof Query) {
	case `number`:
	...
	const attachment:MessageAttachment = new MessageAttachment(`./Pokèbot/Pokemon/1-151/250px-${pokemon[dexNumber].nat}${pokemon[dexNumber].name}.png`);
	const embed:MessageEmbed = new MessageEmbed()
		.setColor(pokemon[dexNumber].types[0].color as ColorResolvable)
		.setTitle(pokemon[dexNumber].name)
		.addFields({ name: `National dex number:`, value: pokemon[dexNumber].nat },
